...
import { collection, addDoc, serverTimestamp, onSnapshot, query, where, Timestamp } from 'firebase/firestore';
...
// 1. 検索パラメータを安全に扱うためのメインコンポーネント
export default function UploadPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-indigo-50 flex items-center justify-center font-black text-slate-400">読み込み中...</div>}>
      <UploadContent />
    </Suspense>
  );
}

// 2. 実際のアップロード処理を行うコンポーネント
function UploadContent() {
  ...
  useEffect(() => {
    ...
    return () => unsub();
  }, [submittedTaskIds, classId, selectedTaskId]);
  ...
  useEffect(() => {
    const startCamera = async () => {
      try {
        ...
        alert("カメラが起動できませんでした。ブラウザの設定で許可してください。");
      }
    };
  ...
  const takePhoto = () => {
    ...
    if (v && c) {
      ...
      if (ctx) {
        ctx.drawImage(v, 0, 0);
        setImages(prev => [...prev, c.toDataURL('image/jpeg', 0.8)]); // 画質を少し落として容量を節約
      }
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedTaskId || images.length === 0) return alert("作品を撮影し、課題箱を選択してください");
    ...
    try {
      ...
      await addDoc(collection(db, "works"), submission);
      alert("提出完了しました！");
      router.push('/student');
    } catch (err) { 
      console.error(err);
      ...
      <div className="min-h-screen bg-black text-white flex flex-col font-sans">
        ...
        <div className="flex-1 relative flex items-center justify-center overflow-hidden">
          ...
          <div className="absolute bottom-10 flex items-center justify-center w-full gap-8">
            <button 
              onClick={takePhoto} 
              className="w-24 h-24 bg-white rounded-full border-[10px] border-white/30 active:scale-90 transition-transform shadow-2xl" 
            />
          </div>
        </div>

        <div className="h-40 bg-zinc-900 p-4 flex gap-3 overflow-x-auto items-center border-t border-white/10">
          {images.map((img, i) => (
            <div key={i} className="relative flex-shrink-0 animate-in zoom-in duration-300">
              ...
              <button 
                ...
                className="absolute -top-2 -right-2 bg-red-500 w-6 h-6 rounded-full text-[10px] flex items-center justify-center font-bold shadow-lg"
              >✕</button>
            </div>
          ))}
          {images.length > 0 && (
            <button 
              ...
              className="ml-auto bg-indigo-600 px-8 py-4 rounded-[24px] font-black text-sm shadow-[0_0_30px_rgba(79,70,229,0.5)] active:scale-95 transition-all"
            >プレビューへ →</button>
          )}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-6 text-slate-900 font-sans">
      <header className="max-w-md mx-auto mb-8 flex justify-between items-center">
        <button onClick={() => setStep('shoot')} className="flex items-center gap-1 text-xs font-black text-slate-400 underline decoration-slate-200 underline-offset-4 uppercase tracking-tighter">
          <IoChevronBack /> 撮り直す
        </button>
      ...
      <div className="max-w-md mx-auto space-y-8 pb-24">
        ...
        <div className="flex gap-4 overflow-x-auto py-2 snap-x px-2">
          {images.map((img, i) => (
            <img 
              ...
              className="h-72 rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border-[6px] border-white flex-shrink-0 snap-center transition-all" 
              alt="preview" 
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 bg-white p-5 rounded-[32px] shadow-sm border border-slate-100">
          <div className="col-span-1 border-r border-slate-100 pr-2">
            ...
            <input 
              ...
              className="w-full bg-transparent font-black text-2xl p-0 border-none focus:ring-0 placeholder:text-slate-100" 
            />
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3">
            {tasks.length > 0 ? tasks.map((t) => (
              ...
              <div className="p-12 text-center text-sm font-bold text-slate-300 bg-white rounded-[40px] border-4 border-dashed border-slate-50 italic">
                提出可能な課題はありません
              </div>
            )}
          </div>
        ...
        {/* コメント入力 */}
        <textarea 
          placeholder="作品についてのメッセージ（空欄でもOK）" 
          value={comment} 
          onChange={(e) => setComment(e.target.value)} 
          className="w-full p-8 bg-white rounded-[40px] font-bold shadow-sm h-40 border-none focus:ring-4 focus:ring-indigo-50/50 text-slate-700 placeholder:text-slate-200" 
        />
        ...
        <button 
          ...
          className="w-full py-7 bg-slate-900 text-white rounded-[48px] font-black italic text-3xl shadow-[0_20px_40px_rgba(0,0,0,0.2)] active:scale-95 disabled:bg-slate-200 disabled:shadow-none transition-all uppercase tracking-tighter"
        >
          {loading ? "送信中..." : "作品を提出する"}
        </button>
      </div>
