...
import { db, auth } from '../../../firebase/firebase'; // 🚀 パス修正
...
export default function StudentHistory() {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!confirm("この提出を取り消しますか？作品データが削除されます。")) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 text-slate-900 font-sans">
      <header className="max-w-md mx-auto mb-10 flex justify-between items-center">
        ...
        {/* 🚀 戻り先を /student に修正しました */}
        <Link href="/student" className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm border border-slate-100 font-black text-slate-200 hover:text-indigo-600 transition-all text-xl">←</Link>
      </header>

      <div className="max-w-md mx-auto space-y-6 pb-20">
        {works.length === 0 ? (
          <div className="text-center py-40 border-2 border-dashed border-slate-200 rounded-[48px]">
            <p className="font-black italic text-slate-300 uppercase tracking-widest">No history yet.</p>
          </div>
        ) : (
          works.map((w) => (
            <div key={w.id} className="bg-white p-8 rounded-[48px] border border-white shadow-sm relative overflow-hidden group">
              ...
              <div className="mb-4">
                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-1 italic">
                  {w.taskName || "Untitled Task"}
                </p>
                <p className="text-[9px] font-bold text-slate-300 uppercase tracking-tighter">
                  {w.createdAt?.toDate().toLocaleDateString() || "Recent"}
                </p>
              </div>
              ...
              <div className="mt-4 px-2">
                <p className="text-sm font-bold text-slate-600 leading-relaxed italic">
                  {w.comment ? `“ ${w.comment} ”` : "No message."}
                </p>
              </div>
            </div>
          ))
      ...
      {selectedImg && (
        ...
        >
          <img src={selectedImg} className="max-w-full max-h-[80vh] object-contain rounded-[40px] shadow-2xl animate-in zoom-in duration-300" />
          <p className="absolute bottom-12 text-white/30 font-black italic uppercase tracking-widest text-[10px]">Tap to close</p>
        </div>
      )}
    </div>
  );
