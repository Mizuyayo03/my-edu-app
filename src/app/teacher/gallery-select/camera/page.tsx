...
export default function CameraShare() {
  ...
  const classId = searchParams.get('classId'); // 🔴重要：URLに ?classId=xxx が必要
  ...
  const handleSubmit = async () => {
    if (!classId) return alert("クラスIDが不明です。前の画面からやり直してください。");
    if (!capturedImage || !title) return alert("タイトルと写真が必要です");

    setLoading(true);
    try {
      await addDoc(collection(db, "shared_resources"), {
        title: title,
        imageUrl: capturedImage, // 🔵生徒側もこの名前で読み取る
        classId: classId,
        teacherId: user.uid,
        type: 'camera',
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col">
      <header className="p-6 flex justify-between items-center bg-black/20">
        ...
        <span className="text-[10px] font-black uppercase text-indigo-400 tracking-widest">Camera Share</span>
      </header>
      <main className="flex-1 flex flex-col items-center justify-center p-4">
        {!capturedImage ? (
          ...
          <div className="w-full max-w-md space-y-6">
            ...
            <input 
              type="text" placeholder="タイトルを入力..." value={title} 
